...
//        if (!APIClient.getAPIClient(context, getSrc()).checkAPISupport(getType(), getArgsSource())) {
//            return Util.futureResult(
//                    "Transaction " + getType() + " not supported for " + getSrc()
//            );
//        }
//        return applyLocally(context)
//                .thenCompose(aVoid -> applyViaAPI(context));
//    }

    // Optimistically apply changes locally
    public abstract CompletableFuture<Void> applyLocally(Context context);
//    // Apply changes on the actual backend
//    abstract CompletableFuture<Void> applyViaAPI(Context context);
